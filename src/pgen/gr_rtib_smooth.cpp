...
Real SmoothInterpolationDeriviative(Real t, Real lower_value, Real higher_value, Real width){
  return 0.5 * (higher_value - lower_value) * ( SQR(1.0/std::cosh(t/width))/width );
}
...
void Pressure_ODE_2D(Real t, Real y, bool is_top,ParameterInput *pin, MeshBlock *pmb, Real *dydt) {
      ...
      Real v_c = shear_velocity/2.0;
      Real v_h = -shear_velocity/2.0;
...
void integrate_P_ODE(int il, int iu, int jl, int ju, int kl, int ku, AthenaArray<Real> x_coord,MeshBlock *pmb,ParameterInput *pin,AthenaArray<Real> &P_sol ){
 ...
 //Integrate the pressure eqation by starting at the boundary and then integrate by roughly 10 times 
 //smaller steps
  ...
  for(int k=ks; k<=ke; k++) {
    for(int j=js; j<=je; j++) {
      ...
      for(int i=is; i<=ie; i++) {
        ...
        // vsq+= pmb->phydro->w(IDN,k,j,i) * ( SQR(v2) + SQR(v3) ); //weight by density
        vsq+= ( SQR(v2) + SQR(v3) ); 
    ...
    for (int k=kl; k<=ku; k++) {
      for (int j=jl; j<=ju; j++) {
        ...
        for (int i=il; i<=iu; i++) {
          ...
          Real v1 = SmoothInterpolation(pcoord->x2v(j), -shear_velocity/2.0, shear_velocity/2.0, length_of_rotation_region) ;
          Real v2 = 0.0;
          Real v3 = 0.0;
    ...
    for (int k=kl; k<=ku; k++) {
      for (int j=jl; j<=ju; j++) {
        ...
        for (int i=il; i<=iu; i++) {
          ...
          Real v1 = SmoothInterpolation(pcoord->x3v(k), -shear_velocity/2.0, shear_velocity/2.0, length_of_rotation_region) ;
          Real v2 = 0.0;
          Real v3 = 0.0;
      ...
      for (int k=ks; k<=ke; k++) {
        for (int j=js; j<=je; j++) {
          ...
          for (int i=is; i<=ie+1; i++) {
            ...
            Bmag = std::sqrt( P_sol(k) * (1/beta)*2.0 );
